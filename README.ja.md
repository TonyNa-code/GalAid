...
単体の `.zip/.rar/.7z/.lzh/.lha/.arj/.cab/.tar/.tgz/.tar.gz/.iso/.cue/.bin` は Web 版でもパッケージ段階として識別できます。`.mds/.mdf`、`.ccd/.img/.sub`、`.nrg`、BlindWrite、`.mdx`、`.daa`、`.uif`、`.pdi` などの古いディスクイメージも扱います。デスクトップ beta は ZIP ディレクトリメタデータを確認し、同梱またはローカル 7z ツールで RAR/7z/LZH/LHA/ARJ/CAB/TAR 系のディレクトリメタデータも一覧化できます。またインストールディスク、修正パッチ、特典ディスク、DirectX/VC++/RPG Maker RTP 修復項目、分割ファイル、ディスクイメージ種別も判定します。ユーザーが起動画面のメインボタンを押すと、GalAid は既定でパッケージ横に新しい `*-prepared` フォルダを作り、必要なら既知のパスワードを求め、ローカル展開を行います。`.tar.gz/.tgz` のような圧縮 tar は二段階で展開し、Windows ISO マウント、または一般的なイメージ展開も行い、再スキャン後に推奨入口を起動します。処理後に `setup.exe/install.exe/autorun.exe/.msi`、または `autorun.inf` が指す `Start.exe/SetupJP.exe/Setup.cmd` などの入口だけが見つかる場合は、GalAid がインストールメディア入口として開き、完了後にインストール先フォルダを再投入するよう案内します。出力先を自分で選びたい場合は、従来の「展開して再スキャン」または「マウント/展開して再スキャン」も使えます。起動に失敗した場合は、見えた症状を選ぶだけで手順を更新できます。
