...
- インストーラ、アンインストーラ、パッチ、DirectX/VC++/RPG Maker RTP の修復項目を主プログラムと区別
...
単体の `.zip/.rar/.7z/.lzh/.lha/.arj/.cab/.tar/.tgz/.tar.gz/.iso/.cue/.bin` は Web 版でもパッケージ段階として識別できます。`.mds/.mdf`、`.ccd/.img/.sub`、`.nrg`、BlindWrite、`.mdx`、`.daa`、`.uif`、`.pdi` などの古いディスクイメージも扱います。デスクトップ beta は ZIP ディレクトリメタデータを確認し、同梱またはローカル 7z ツールで RAR/7z/LZH/LHA/ARJ/CAB/TAR 系のディレクトリメタデータも一覧化できます。内部ディレクトリが確認できた自己展開 `.exe` パッケージもアーカイブとして扱い、小さな `.cue` ファイルから `FILE` 行の `.bin/.img` トラックを同じイメージセットへまとめます。7z 互換ツールがディスクイメージのディレクトリを読める場合は、展開せずにイメージ内の起動ファイル、インストーラ、ランタイム修復項目も手がかりとして拾い、`autorun.inf + Start.exe + data1.cab` のような古いインストールディスク入口は事前にインストールメディアルートへ寄せます。さらにインストールディスク、修正パッチ、特典ディスク、分割ファイル、ディスクイメージ種別、DOS/Win16/Win32/Win64 実行ファイルヘッダー、古い Win32 subsystem バージョン、DirectDraw/DirectSound/DirectInput/WinMM/VC++/VB6/.NET/QuickTime/Borland などの古い import ヒントも判定します。ユーザーが起動画面のメインボタンを押すと、GalAid は既定でパッケージ横に新しい `*-prepared` フォルダを作り、必要なら既知のパスワードを求め、ローカル展開を行います。`.tar.gz/.tgz` のような圧縮 tar は二段階で展開し、識別済みの自己展開 EXE は先に展開して再スキャンし、Windows ISO マウント、または一般的なイメージ展開も行い、再スキャン後に推奨入口を起動します。推奨入口が DOS COM/MZ、Win16 NE、または古い LE/LX 形式なら、GalAid は直接起動せず DOSBox、旧 OS、仮想マシンのルートへ切り替えます。Win95/Win2000/XP 世代の Win32 入口なら、まず起動を試せますが、失敗後は XP/Win98 互換モード、全画面最適化の無効化、短い英数字パス、古い DirectX/Runtime/動画コンポーネントを優先して確認するよう案内します。処理後に `setup.exe/install.exe/autorun.exe/.msi`、または `autorun.inf` が指す `Start.exe/SetupJP.exe/Setup.cmd` などの入口だけが見つかる場合は、GalAid がインストールメディア入口として開き、完了後にインストール先フォルダを再投入するよう案内します。出力先を自分で選びたい場合は、従来の「展開して再スキャン」または「マウント/展開して再スキャン」も使えます。起動に失敗した場合は、見えた症状を選ぶだけで手順を更新できます。
