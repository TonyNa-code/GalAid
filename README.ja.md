...
単体の `.zip/.rar/.7z/.iso/.cue/.bin` は Web 版でもパッケージ段階として識別できます。デスクトップ beta は ZIP ディレクトリメタデータを確認し、同梱またはローカル 7z ツールで RAR/7z のディレクトリメタデータも一覧化できます。また `.iso/.cue/.bin` などのディスクイメージ種別も判定します。ユーザーが「展開して再スキャン」または「マウント/展開して再スキャン」を押した場合は、出力先を選び、既に知っているパスワードを入力し、ローカル展開、Windows ISO マウント、または一般的なイメージの展開後に自動で再スキャンできます。準備完了後は起動画面で推奨入口を強調し、ユーザーがファイル一覧を探し直さなくて済むようにします。
...
Web 版はブラウザが許可したファイルメタデータのみを読み取ります。デスクトップ beta はスキャン時に絶対パスを見る場合がありますが、レポートとサポートバンドルでは相対パスを既定にします。展開、イメージ展開、ISO マウントはユーザーが明示的に操作した場合だけ行います。パスワードはそのローカル展開にだけ使われ、レポートには保存されません。GalAid はゲームファイルをアップロード、密かに実行、原本変更、未知パスワードの復号をしません。
