...
- デスクトップ ZIP 事前チェックは ZIP のディレクトリ情報のみを読み取り、ゲームファイルを展開しない
...
単体の `.zip/.rar/.7z/.iso/.cue/.bin` は Web 版でもパッケージ段階として識別できます。デスクトップ beta は ZIP のディレクトリメタデータを追加で確認し、内部の起動候補や構造の手がかりを見つけられますが、展開や内容読み取りは行いません。
...
Web 版はブラウザが許可したファイルメタデータのみを読み取ります。デスクトップ beta はスキャン時に絶対パスを見る場合がありますが、レポートとサポートバンドルでは相対パスを既定にします。GalAid はゲームファイルをアップロード、実行、変更、復号、展開しません。
