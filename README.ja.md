...
- デスクトップのパッケージ/イメージ事前チェックは ZIP ディレクトリ、ローカル 7z/7zz による RAR/7z メタデータ、ディスクイメージ種別を扱い、ユーザー確認後にローカル展開と再スキャンもできる
- Markdown 診断レポートのコピー/ダウンロード
- ゲームファイルや内容を含まないサポートバンドルの出力
...
単体の `.zip/.rar/.7z/.iso/.cue/.bin` は Web 版でもパッケージ段階として識別できます。デスクトップ beta は ZIP ディレクトリメタデータを確認し、ローカルに 7z/7zz がある場合は RAR/7z のディレクトリメタデータも一覧化できます。また `.iso/.cue/.bin` などのディスクイメージ種別も判定します。ユーザーが「展開して再スキャン」を押した場合は、出力先を選び、既に知っているパスワードを入力し、ローカル展開後に自動で再スキャンできます。

## 安全境界

GalAid はゲーム、クラック、DRM 回避、シリアル、復号キー、未許可のアセット抽出を提供しません。
...
- 起動/ランタイム問題の理解
- 個人アーカイブの整理
- 制作者や翻訳者が自分のプロジェクトフォルダを確認する作業

Web 版はブラウザが許可したファイルメタデータのみを読み取ります。デスクトップ beta はスキャン時に絶対パスを見る場合がありますが、レポートとサポートバンドルでは相対パスを既定にします。展開はユーザーが明示的に操作し、出力先を選んだ場合だけ行います。パスワードはそのローカル展開にだけ使われ、レポートには保存されません。GalAid はゲームファイルをアップロード、密かに実行、原本変更、未知パスワードの復号、マウントしません。
