...
- デスクトップスキャンで `.exe/.com` ヘッダーを読み、DOS COM/MZ、Win16 NE、古い LE/LX、Win32、Win64 を区別して、古い入口を通常の Windows 実行ファイルとして誤起動しない
