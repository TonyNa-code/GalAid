...
- デスクトップのパッケージ/イメージ事前チェックは ZIP ディレクトリ、ローカル 7z/7zz による RAR/7z メタデータ、ディスクイメージ種別を扱い、展開やマウントは行わない
...
単体の `.zip/.rar/.7z/.iso/.cue/.bin` は Web 版でもパッケージ段階として識別できます。デスクトップ beta は ZIP ディレクトリメタデータを確認し、ローカルに 7z/7zz がある場合は RAR/7z のディレクトリメタデータも一覧化できます。また `.iso/.cue/.bin` などのディスクイメージ種別も判定します。内部の起動候補、インストーラー、分割ボリューム状態、構造の手がかりを見つけられますが、展開、マウント、アップロード、内容読み取りは行いません。
...
Web 版はブラウザが許可したファイルメタデータのみを読み取ります。デスクトップ beta はスキャン時に絶対パスを見る場合がありますが、レポートとサポートバンドルでは相対パスを既定にします。GalAid はゲームファイルをアップロード、実行、変更、復号、展開、マウントしません。
