  ...
  },
  {
    ...
    step: {
      "zh-CN": {
        ...
        action: "先贴弹窗/日志；如果没有文本，优先尝试日区/Locale Emulator、英文短路径、完整解压和 DirectX/DirectPlay/VC++ 运行库。",
      },
      en: {
        title: "Collect crash evidence",
        detail: "Manually marked: the game exits immediately after launch.",
        action: "Paste any dialog/log text. If there is no text, try Japanese locale / Locale Emulator, a short ASCII path, full extraction, and DirectX/DirectPlay/VC++ runtimes.",
      },
      ja: {
        title: "クラッシュ情報を集める",
        detail: "手動入力: 起動直後に終了します。",
        action: "ダイアログやログがあれば貼り付けます。本文がない場合は、日本語 locale / Locale Emulator、短い英数字パス、完全展開、DirectX/DirectPlay/VC++ ランタイムを確認してください。",
      },
    },
  },
  {
  ...
  },
  {
    ...
    step: {
      "zh-CN": {
        ...
        action: "把完整 DLL 名贴到错误信息里；d3dx/xinput 多半看 DirectX，dplayx/dpnet 看 DirectPlay，msvcr/msvcp/vcruntime 多半看 VC++，RGSS/RTP 看 RPG Maker RTP。",
      },
      en: {
        title: "Missing DLL or runtime",
        detail: "Manually marked: launch failure may be caused by a missing DLL or runtime.",
        action: "Paste the exact DLL name. d3dx/xinput usually points to DirectX, dplayx/dpnet to DirectPlay, msvcr/msvcp/vcruntime to VC++, and RGSS/RTP to RPG Maker RTP.",
      },
      ja: {
        title: "DLL/ランタイム不足",
        detail: "手動入力: DLL またはランタイム不足の可能性があります。",
        action: "正確な DLL 名を貼り付けてください。d3dx/xinput は DirectX、dplayx/dpnet は DirectPlay、msvcr/msvcp/vcruntime は VC++、RGSS/RTP は RPG Maker RTP が主な確認先です。",
      },
    },
  },
];
      ...
      },
      {
        ...
        step: {
          "zh-CN": {
            ...
            action: "把完整报错贴到左侧错误信息框，再重新更新路线；DLL 名、日文乱码、RTP、DirectX、DirectPlay、VC++ 字样都很关键。",
          },
          en: {
            title: "Paste the copyable error first",
            detail: "The triage answer says a dialog or log text is visible.",
            action: "Paste the full error into the error box and update the route again. DLL names, mojibake, RTP, DirectX, DirectPlay, and VC++ wording matter.",
          },
          ja: {
            title: "コピーできるエラーを先に貼る",
            detail: "問診ではダイアログまたはログ本文が見えている状態です。",
            action: "エラー全文を左のエラー欄に貼り付け、手順を更新してください。DLL 名、文字化け、RTP、DirectX、DirectPlay、VC++ の語が重要です。",
          },
        },
      },
      {
...
const ASSISTANT_LANGUAGE_PACKS = {
  "zh-CN": {
    ...
    ui: {
      ...
      clear: "清空",
      errorLabel: "错误信息",
      errorPlaceholder: "粘贴报错文本，例如 d3dx9_43.dll、dplayx.dll、乱码、RTP、VCRUNTIME...",
      ocrImage: "识别报错截图",
      ...
      safeModeTitle: "启动方式",
      safeModeBody: "网页里会生成可检查的命令；桌面版可以在你点击后用正确工作目录启动本地 .exe/.com/.bat/.cmd/.lnk，安装盘 .msi 也可从安装入口打开。",
      entryLabel: "entry",
      ...
      runtimeAssistantBody: "检测这台 Windows 电脑上的 DirectX、DirectPlay、VC++、.NET、VB6、QuickTime/旧视频组件、RPG Maker RTP 和系统区域。",
  ...
  en: {
    ...
    ui: {
      ...
      errorPlaceholder: "Paste an error such as d3dx9_43.dll, dplayx.dll, mojibake, RTP, VCRUNTIME...",
      ...
      safeModeBody: "The web app generates inspectable commands. In the desktop app, a deliberate click can launch local .exe/.com/.bat/.cmd/.lnk entries with the correct working directory, while install-media .msi entries open through the installer route.",
      entryLabel: "entry",
      workdirLabel: "workdir",
      desktopPathReady: "desktop path ready",
      relativePathOnly: "relative path only",
      ...
      runtimeAssistantBody: "Checks this Windows PC for legacy DirectX files, DirectPlay, VC++ redistributables, .NET, VB6, QuickTime/video components, RPG Maker RTP, and locale state.",
  ...
  ja: {
    ...
    ui: {
      ...
      clear: "クリア",
      errorLabel: "エラー本文",
      errorPlaceholder: "d3dx9_43.dll、dplayx.dll、文字化け、RTP、VCRUNTIME などのエラーを貼り付けます...",
      ocrImage: "画像から読む",
      ...
      safeModeTitle: "起動方法",
      safeModeBody: "Web 版では確認できるコマンドを生成します。デスクトップ版では、クリックした場合に正しい作業フォルダでローカルの .exe/.com/.bat/.cmd/.lnk を起動でき、インストールメディアの .msi も開けます。",
      entryLabel: "入口",
      workdirLabel: "作業フォルダ",
      ...
      runtimeAssistantBody: "この Windows PC の古い DirectX、DirectPlay、VC++、.NET、VB6、QuickTime/動画コンポーネント、RPG Maker RTP、locale 状態を確認します。",
...
function isAllowedDesktopScriptLaunch(file) {
  if (!["bat", "cmd"].includes(file?.ext)) return true;
  const lower = String(file.lowerPath || file.path || file.name || file.fullPath || "").toLowerCase();
  return !(
    /(^|\/)(setup|install|installer|redist|support|patch|update|unins|uninstall)([._ -]|$)/i.test(lower) ||
    /^(setup|install|installer|redist|patch|update|unins|uninstall)[\w .-]*\.(bat|cmd)$/i.test(String(file.name || "")) ||
    /(dxsetup|dxwebsetup|vcredist|vc_redist|dotnet|config|setting|option|keygen|crack|serial|no.?dvd|no.?cd|免dvd|免cd)/i.test(lower)
  );
}
