...
The launch page now starts with a one-stop guide: import, prepare, launch, then collect failure evidence if the game still does not open. In the desktop beta, the primary button can prepare a trusted archive/image into a fresh `*-prepared` folder beside the package, rescan it, and launch the top entry without sending the user through the package tab. When the pasted error points to DirectX, VC++, or RPG Maker RTP and a matching bundled repair tool is present, the same guide prioritizes opening that repair tool before retrying the game launcher. The `路线` tab combines the same archive/image state, launch candidates, runtime checks, error recipes, and engine clues into an ordered checklist. It can be copied as Markdown and is also included in support bundles as `roadmap.json` and `roadmap-checklist.md`.
