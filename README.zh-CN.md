...
- 桌面包/镜像预检可读取 ZIP 目录、通过内置或本机 7z 工具列出 RAR/7z 元数据，并识别镜像介质线索和包内运行库修复项；用户点击一键启动后可本地解压、挂载/解包镜像、自动重扫并启动推荐入口
- 复制或下载 Markdown 报告
...
单个 `.zip/.rar/.7z/.iso/.cue/.bin` 可以在网页里被识别为包或镜像阶段，也包括 `.mds/.mdf`、`.ccd/.img/.sub`、`.nrg`、BlindWrite、`.mdx`、`.daa`、`.uif`、`.pdi` 这类古早镜像。桌面 beta 可以额外预检 ZIP 目录元数据、通过内置或本机 7z 工具列出 RAR/7z 目录元数据，并识别安装盘、补丁包、特典盘、DirectX/VC++/RPG Maker RTP 修复项、分卷和镜像介质角色。用户点击启动页主按钮后，GalAid 会默认在压缩包旁边创建新的 `*-prepared` 目录，必要时提示输入已知密码，然后本地解压、在 Windows 上挂载 `.iso`，或尽力解包常见镜像，重扫后直接启动推荐入口。手动的“解压并重扫”或“挂载/解包并重扫”仍然保留，适合想自己选择输出位置的用户。如果启动仍失败，也可以直接勾选现象来更新路线。
