...
单个 `.zip/.rar/.7z/.iso/.cue/.bin` 可以在网页里被识别为包或镜像阶段。桌面 beta 可以额外预检 ZIP 目录元数据，发现压缩包内部可能的启动器和引擎/结构线索，但不会解压或读取文件内容。

## 安全边界

GalAid 不提供游戏、破解、DRM 绕过、序列号、解密密钥或未授权素材提取。
...
网页版本只读取浏览器暴露的文件元数据。桌面 beta 在扫描时可能内部看到绝对路径，但报告和求助包默认使用相对路径。GalAid 不上传、不运行、不修改、不解密、不解压游戏文件。
