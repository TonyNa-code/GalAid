...
- 桌面包/镜像预检可读取 ZIP 目录、通过本机 7z/7zz 列出 RAR/7z 元数据，并识别镜像介质线索，但不解压、不挂载游戏文件
- 复制或下载 Markdown 报告
- 导出不包含游戏文件和文件内容的求助包
...
单个 `.zip/.rar/.7z/.iso/.cue/.bin` 可以在网页里被识别为包或镜像阶段。桌面 beta 可以额外预检 ZIP 目录元数据、在本机存在 7z/7zz 时列出 RAR/7z 目录元数据，并识别 `.iso/.cue/.bin` 这类镜像介质角色。它可以发现内部启动器、安装器、分卷状态和引擎/结构线索，但不会解压、挂载、上传或读取游戏文件内容。
...
网页版本只读取浏览器暴露的文件元数据。桌面 beta 在扫描时可能内部看到绝对路径，但报告和求助包默认使用相对路径。GalAid 不上传、不运行、不修改、不解密、不解压、不挂载游戏文件。
